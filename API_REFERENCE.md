...
15. [Colecciones](#colecciones)
16. [Fecha y Hora](#fecha-y-hora)
17. [Controles de Firma](#controles-de-firma)
18. [Navegación y App Shell](#navegación-y-app-shell)
19. [Feedback y Estado](#feedback-y-estado)
20. [Texto y Contenido Rico](#texto-y-contenido-rico)
21. [Internacionalización (i18n)](#internacionalización-i18n)
22. [Espaciado y Cursor](#espaciado-y-cursor)
23. [DPI](#dpi)
24. [Style Overrides](#style-overrides)
25. [Accesibilidad](#accesibilidad)
26. [Temas y Estilos](#temas-y-estilos)
27. [Callbacks y Eventos](#callbacks-y-eventos)
28. [Iconos en Widgets](#iconos-en-widgets)
...
Colecciones a escala de aplicación: mosaico de tiles, grilla de datos editable,
paginación, acordeón y carrusel. `GridView` y `DataGrid` virtualizan (solo
construyen lo visible), por lo que aguantan miles de items.

### GridView

Mosaico de items que reflujen según el ancho. `itemBuilder(index)` construye cada
celda visible. Con `minItemWidth > 0` las columnas son fluidas (se calculan del
ancho disponible); si no, se usan columnas fijas derivadas de `itemSize`.
...
```cpp
GridView("galeria", productos.size(), {160, 200},
    [&](int i){
        Image(("img" + std::to_string(i)), productos[i].textura, {160, 120});
        Label(productos[i].nombre);
    },
    12.0f, 160.0f /* columnas fluidas */);
```

### DataGrid

Grilla de datos con columnas ordenables/redimensionables y edición inline por tipo.
Virtualiza por filas. El acceso a celdas es por callbacks (`getCell`/`setCell`); los
índices del resultado están en el orden **lógico** original del caller.

```cpp
struct DataColumn {
    std::string header;
    float width = 120.0f;
    bool sortable = true, resizable = true, editable = false;
    enum class Type { Text, Number, Bool, Choice } type = Type::Text;
    std::vector<std::string> choices;   // para Type::Choice
};
struct DataGridResult {
    int sortedColumn = -1; bool ascending = true;
    int editedRow = -1, editedCol = -1;  // celda confirmada este frame, o -1
};
...
std::vector<DataColumn> cols = {
    { "SKU",      100 },
    { "Producto", 220 },
    { "Stock",    80,  true, true, true, DataColumn::Type::Number },
    { "Activo",   70,  true, true, true, DataColumn::Type::Bool   },
};
auto r = DataGrid("inv", cols, filas.size(),
    [&](int row, int col){ return celdaTexto(row, col); },
    [&](int row, int col, const std::string& v){ commitCelda(row, col, v); });
if (r.sortedColumn >= 0) ordenar(r.sortedColumn, r.ascending);
```

### Pagination

Controles ‹ 1 2 … 9 › con colapso de intermedios. Devuelve la página seleccionada
(0-based).
...
```cpp
static int pagina = 0;
pagina = Pagination("pag", totalPaginas, &pagina);
```

### ExpanderList

Lista vertical de cabeceras colapsables. Con `accordion = true`, abrir una cierra
las demás.
...
```cpp
ExpanderList("faq", preguntas.size(),
    [&](int i){ return preguntas[i].titulo; },
    [&](int i){ Label(preguntas[i].respuesta); },
    true /* acordeón */);
```

### FlipView

Muestra un item a la vez con navegación (flechas / indicadores). Devuelve el índice
actual.
...
```cpp
static int slide = 0;
slide = FlipView("carrusel", imagenes.size(),
    [&](int i){ Image(("s"+std::to_string(i)), imagenes[i], {480, 270}); }, &slide);
```
...
Texto de solo lectura que el usuario puede seleccionar y copiar (Ctrl+C al portapapeles del SO). Doble-click selecciona palabra, triple-click la línea; Shift+flechas extiende.
...
```cpp
SelectableText("log", salidaDelProceso);   // el usuario puede copiar el texto
```
...
Formato por locale (números, moneda, fecha/hora), catálogo de strings traducibles
con pluralización, y layout RTL. Declarado en `core/I18n.h`.

### Locale y formato

```cpp
struct LocaleFormat { /* separadores, currencySymbol, currencySymbolBefore, … */ };
...
```cpp
SetLocale("es-ES");
Label(FormatCurrency(1234.5));     // "1.234,50 €" según el locale
Label(FormatDate(2026, 7, 3));
```

### Strings traducibles

```cpp
void RegisterTranslations(const std::string& locale, /* mapa clave→texto */);
...
std::string Tr(const std::string& key);                       // traduce (o devuelve la clave)
std::string TrFormat(const std::string& key, const std::string& arg);  // con un {} sustituido
std::string TrPlural(const std::string& keyBase, long long count);     // forma plural según count
```

```cpp
LoadTranslationsFromFile("es-ES", "lang/es.json");
SetLocale("es-ES");

Label(Tr("menu.file"));                       // "Archivo"
Label(TrPlural("items.count", n));            // "1 elemento" / "5 elementos"
if (Button(Tr("action.save"))) { /* … */ }
```
